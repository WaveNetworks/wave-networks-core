...
foreach ([192, 512] as $size) {
    $png = $uploads . "/pwa_icon_{$size}.png";
    if (file_exists($png)) {
        $icons[] = [
            'src'     => "uploads/pwa_icon_{$size}.png",
            'sizes'   => "{$size}x{$size}",
            'type'    => 'image/png',
            'purpose' => 'any',
        ];
...
if (!empty($b['pwa_screenshot_wide'])) {
    $sw_path = $uploads . '/' . $b['pwa_screenshot_wide'];
    if (file_exists($sw_path)) {
        $sw_info = @getimagesize($sw_path);
        $screenshots[] = [
            'src'         => 'uploads/' . $b['pwa_screenshot_wide'],
...
if (!empty($b['pwa_screenshot_mobile'])) {
    $sm_path = $uploads . '/' . $b['pwa_screenshot_mobile'];
    if (file_exists($sm_path)) {
        $sm_info = @getimagesize($sm_path);
        $screenshots[] = [
            'src'   => 'uploads/' . $b['pwa_screenshot_mobile'],
