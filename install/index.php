...
// ─── Prevent running if config already exists AND install is complete ────────
$configPath = __DIR__ . '/../config/config.php';
if (file_exists($configPath)) {
    // Config exists — check if an admin user has been created (install complete)
    ...
    try {
        ...
        if (!empty($tables)) {
            $installComplete = (int)$checkPdo->query("SELECT COUNT(*) FROM user")->fetchColumn() > 0;
        }
        $checkPdo = null;
    } catch (PDOException $e) {
        // DB not ready yet — allow installer to show step 2
    }
...
// If config already exists (e.g. auto-generated by Docker entrypoint), skip to step 2
if (file_exists($configPath)) {
    $step = 'user';
}
    ...
    if (empty($errors)) {
        try {
            ...
            if (empty($errors)) {
                ...
                $stmt = $pdo->prepare("INSERT INTO user (email, password, shard_id, is_admin, is_confirmed, created_date) VALUES (?, ?, ?, 1, 1, ?)");
